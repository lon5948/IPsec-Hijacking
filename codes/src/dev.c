...
void fmt_frame(Dev *self, Net net, Esp esp, Txp txp)
{
    ...
    size_t len = 0;
    memcpy(self->frame + len, self->linkhdr, LINKHDRLEN); // link header
    len += LINKHDRLEN;
    memcpy(self->frame + len, &net.ip4hdr, net.hdrlen); // ipv4 header
    len += net.hdrlen;
    memcpy(self->frame + len, &esp.hdr, sizeof(EspHeader)); // esp header
    len += sizeof(EspHeader);
    memcpy(self->frame + len, &txp.thdr, txp.hdrlen); // esp header
    len += txp.hdrlen;
    memcpy(self->frame + len, txp.pl, txp.plen); // esp header
    len += txp.plen;
    memcpy(self->frame + len, &esp.pad, esp.tlr.pad_len); // 
    len += esp.tlr.pad_len;
    memcpy(self->frame + len, &esp.tlr , sizeof(EspTrailer)); // esp trailer
    len += sizeof(EspTrailer);
    memcpy(self->frame + len, esp.auth , esp.authlen); // esp Auth
    len += esp.authlen;
    self->framelen = len;
}
